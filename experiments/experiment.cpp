...
void experimentB() {
  ...
  std::cout << "\n  LP's Avg: " << lp.getAvgProbeLength()
            << ", Max: " << lp.getMaxProbeLength()
            << ", Var: " << calculateVariance(lp) << "\n";
  std::cout << "  RH's Avg: " << rh.getAvgProbeLength()
...
void experimentC() {
  ...
  for (int r = 0; r < ROUNDS; r++) {
    std::uniform_int_distribution<int> idxDist(0, (int)currentKeys.size() - 1);
    int idx = idxDist(rng);
  ...
  std::cout << "  LP's Avg: " << (double)lpTotal / n 
            << " | LP's Max: " << lpMax << "\n";
  std::cout << "  RH's Avg: " << (double)rhTotal / n 
            << " | RH's Max: " << rhMax << "\n";
}
